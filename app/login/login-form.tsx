...
export default function LoginForm() {
  ...
  useEffect(() => {
    if (searchParams.get('registered') === '1') {
      toast.success('Registration completed successfully! Please sign in.', 'login-registered');
    }
    if (searchParams.get('reset') === '1') {
      toast.success('Password reset successful. You can now sign in.', 'login-reset');
    }
  }, [searchParams]);
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error('Invalid email or password', 'login-error');
        return;
      }
      ...
      toast.success('You are logged in successfully!', 'login-success');
      router.push('/dashboard');
    } catch {
      toast.error('Something went wrong. Please try again.', 'login-error');
    } finally {
      setLoading(false);
    }
  };
